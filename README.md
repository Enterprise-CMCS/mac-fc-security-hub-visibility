...
## A GitHub Action that syncs AWS Security Hub or Snowflake findings with Jira
...
**_Note that modifications or deletions of labels may lead to duplicate issue creation_**
...
Set `finding-source: snowflake` to read open rows from
`BUS_CMCS.PRIVATE.VW_GLOBAL_SECURITY_FINDINGS` (or another explicitly configured
view). This mode creates one Jira task per stable finding identity:

`TOOL_NAME + FISMA_ID + RESOURCE_ID + FINDING_ID`

The identity is stored as a hashed Jira label, so repeated runs are idempotent.
Every created issue also receives the fixed `snowflake-findings` provenance label
and a canonical FISMA ID and acronym label. Jira reconciliation searches only
the configured project and the exact FISMA label requested by the run, so manual
...
`FISMA_ID`, `FISMA_ACRONYM`, `RESOURCE_ID`, `FINDING_ID`, `TOOL_NAME`,
`STATUS`, `SEVERITY`, `CREATED_AT`, `CLOSED_AT`, `RAW_FINDING`, `FINDING_AGE`,
`NORMALIZED_SEVERITY`, and `SEVERITY_SCORE`.

Exactly one `snowflake-fisma-ids` or `snowflake-fisma-acronyms` value is
mandatory. The action refuses a global-view query without one authoritative
FISMA boundary.
It also fetches one row beyond `snowflake-max-rows` and aborts before Jira
reconciliation if the configured ceiling is exceeded.

Key-pair authentication is the default. Give the service role only the access it
needs; adapt role and warehouse names to your Snowflake account:

```sql
GRANT USAGE ON WAREHOUSE TEAM_CMCS_WH TO ROLE MACFC_JIRA_SYNC_ROLE;
GRANT USAGE ON DATABASE BUS_CMCS TO ROLE MACFC_JIRA_SYNC_ROLE;
GRANT USAGE ON SCHEMA BUS_CMCS.PRIVATE TO ROLE MACFC_JIRA_SYNC_ROLE;
GRANT SELECT ON VIEW BUS_CMCS.PRIVATE.VW_GLOBAL_SECURITY_FINDINGS
  TO ROLE MACFC_JIRA_SYNC_ROLE;
```

Store the private key, passphrase, Snowflake account/user, and Jira token as
GitHub Actions secrets. Do not put them directly in workflow YAML.

```yaml
name: Sync Snowflake security findings to Jira
    ...
    steps:
      - uses: enterprise-cmcs/mac-fc-security-hub-sync@<pinned-commit-sha>
        with:
          ...
          jira-project-key: MACFC
          jira-transition-map: '*:Done'
          jira-api-version: '2'

          snowflake-account: ${{ secrets.SNOWFLAKE_ACCOUNT }}
          snowflake-username: ${{ secrets.SNOWFLAKE_USERNAME }}
...
Start with `dry-run-test-data: 'true'`. Dry run performs the Snowflake read and
Jira search but logs Jira mutations instead of executing them. An inert copyable
workflow template is available at
[`examples/snowflake-view-sync.yml`](examples/snowflake-view-sync.yml); it is not
installed under `.github/workflows`, so adding it does not schedule a sync.
...
- The field name will be: customfield\_<field_id>
